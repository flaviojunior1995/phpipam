...
if( !(preg_match('/^CID[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]$/', $circuit['cid']) ) )
...
# Device Point A must be present
if($circuit['device1'] == "0") 													{ $Result->show("danger", _('Device Point A is mandatory').'!', true); }

# Interface Point A must be present
if($circuit['intdevice1'] == "")	{ $Result->show("danger", _('Interface Point A is mandatory').'!', true); }

# Validate vlan Point A
if( !(($circuit['svlandevice1'] >= 0) && ($circuit['svlandevice1']) <= 4096 && is_numeric($circuit['svlandevice1'])) )	{ $Result->show("danger", _("Invalid S-VLAN Point A"), true); }
if($circuit['svlandevice1'] == "")	{ $Result->show("danger", _('S-VLAN ID Point A is mandatory').'!', true); }
if( !(($circuit['cvlandevice1'] >= 0) && ($circuit['cvlandevice1']) <= 4096 && is_numeric($circuit['cvlandevice1'])) )	{ $Result->show("danger", _("Invalid C-VLAN Point A"), true); }
if($circuit['cvlandevice1'] == "") 	{ $Result->show("danger", _('C-VLAN ID Point A is mandatory').'!', true); }

# Validate vlan Point B
if(!(($circuit['svlandevice2'] >= 0) && ($circuit['svlandevice2']) <= 4096 && is_numeric($circuit['svlandevice2']) || $circuit['svlandevice2'] == "") )	{ $Result->show("danger", _("Invalid S-VLAN Point B"), true); }
if(!(($circuit['cvlandevice2'] >= 0) && ($circuit['cvlandevice2']) <= 4096 && is_numeric($circuit['cvlandevice2']) || $circuit['cvlandevice2'] == "") )	{ $Result->show("danger", _("Invalid C-VLAN Point B"), true); }
