...
$circuits = $Tools->fetch_all_circuits_dwdm($custom_fields);
...
# title
print "<h4>"._('List of DWDM circuits')."</h4>";
print "<hr>";
...
print "<div class='btn-group'>";
    ...
    print "<a href='' class='btn btn-sm btn-default open_popup' data-script='app/admin/circuits/edit-dwdm-circuit.php' data-class='700' data-action='add' data-circuitid='' style='margin-bottom:10px;'><i class='fa fa-plus'></i> "._('Add circuit')."</a>";
	}
print "</div>";
...
print " <th><span class='hidden-sm hidden-xs'>"._('DWDM Channel').'</span></th>';
...
// result
	...
	foreach ($circuits as $circuit) {
		...
		print "	<td class='hidden-xs hidden-sm'>$locationB_html</td>";
                print " <td class='hidden-xs hidden-sm'>$circuit->dwdmchannel</td>";
        ...
        if($User->get_module_permissions ("circuits")>=User::ACCESS_RW) {
            ...
            $links[] = ["type"=>"link", "text"=>_("Edit circuit"), "href"=>"", "class"=>"open_popup", "dataparams"=>" data-script='app/admin/circuits/edit-dwdm-circuit.php' data-class='700' data-action='edit' data-circuitid='$circuit->id'", "icon"=>"pencil"];
        }
        if($User->get_module_permissions ("circuits")>=User::ACCESS_RW) {
            $links[] = ["type"=>"link", "text"=>_("Delete circuit"), "href"=>"", "class"=>"open_popup", "dataparams"=>"  data-script='app/admin/circuits/edit-dwdm-circuit.php' data-class='700' data-action='delete' data-circuitid='$circuit->id'", "icon"=>"times"];
        }
        // print links
