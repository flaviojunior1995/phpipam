...
$circuits = $Tools->fetch_all_circuits_network($custom_fields);
...
# title
print "<h4>"._('List of Network circuits')."</h4>";
print "<hr>";
...
print "<div class='btn-group'>";
    ...
    print "<a href='' class='btn btn-sm btn-default open_popup' data-script='app/admin/circuits/edit-network-circuit.php' data-class='700' data-action='add' data-circuitid='' style='margin-bottom:10px;'><i class='fa fa-plus'></i> "._('Add circuit')."</a>";
	}
print "</div>";
...
print " <th><span class='hidden-sm hidden-xs'>"._('Partner').'</span></th>';
$colspanCustom = 0;
...
// result
	...
	foreach ($circuits as $circuit) {
		...
		print " <td class='hidden-xs hidden-sm'>$circuit->partner</td>";
		//custom
		if(sizeof(@$custom_fields) > 0) {
			foreach($custom_fields as $field) {
				if(!in_array($field['name'], $hidden_circuit_fields)) {
        ...
        if($User->get_module_permissions ("circuits")>=User::ACCESS_RWA) {
            ...
            $links[] = ["type"=>"link", "text"=>_("Edit circuit"), "href"=>"", "class"=>"open_popup", "dataparams"=>" data-script='app/admin/circuits/edit-network-circuit.php' data-class='700' data-action='edit' data-circuitid='$circuit->id'", "icon"=>"pencil"];
        }
        if($User->get_module_permissions ("circuits")>=User::ACCESS_RWA) {
            $links[] = ["type"=>"link", "text"=>_("Delete circuit"), "href"=>"", "class"=>"open_popup", "dataparams"=>"  data-script='app/admin/circuits/edit-network-circuit.php' data-class='700' data-action='delete' data-circuitid='$circuit->id'", "icon"=>"times"];
        }
        // print links
